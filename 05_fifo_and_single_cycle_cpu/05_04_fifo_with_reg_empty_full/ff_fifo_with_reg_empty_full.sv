module ff_fifo_with_reg_empty_full #(
  ...
  localparam pointer_width = $clog2(depth), counter_width = $clog2(depth + 1);
  ...
  always_comb begin
    ...
    // Example
    if (push) wr_ptr_d = wr_ptr_q == max_ptr ? '0 : wr_ptr_q + 1'b1;
    else wr_ptr_d = wr_ptr_q;

    // Task: Add logic for pop to make the FIFO work
    if (pop) rd_ptr_d = rd_ptr_q == max_ptr ? '0 : rd_ptr_q + 1'b1;
  ...
  always_ff @(posedge clk) if (push) data[wr_ptr_q] <= write_data;
